...
var WhiteoutComponent = require('./fds-whiteout');
...
Helper.forEach(growl__closeButtons, function(index, value) {
  ...
  _el.addEventListener('click', function(e){
    // pass associated growl to method
    var _g = Helper.getClosest(e.currentTarget, '.fds-growl');
    growl__dismiss( _g );
  }, false);
});
...
function growl__show(g){
  ...
  if( Helper.hasClass(_growl, 'fds-growl--centered') ){
...
function growl__dismiss(g){
  var _growl = g;
  _growl.className = _growl.className + ' fds-growl--dismissing';
...
function growl__dismissDelay(e){
  ...
  _growl.className = _growl.className.replace(' fds-growl--dismissing','');
