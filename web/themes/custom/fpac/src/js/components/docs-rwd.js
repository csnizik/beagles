...
$('body').on('click', '[data-behavior~="toggle-rwd-size"]', function(event) {
  ...
  $self
    .removeClass('fds-bg:hover--tertiary-100')
    .addClass('fds-bg--secondary-100')
    .attr('aria-selected', true)
    .siblings()
    .removeClass('fds-bg--secondary-100')
    .removeAttr('aria-selected')
    .addClass('fds-bg:hover--tertiary-100')
  ;
...
$('body').on('change', '[data-behavior~="toggle-rwd-table"]', function(event) {
  ...
  $target.toggleClass('fds-table--responsive-horizontal');
