...
Helper.forEach(modal__closeButtons, function (index, value) {
  ...
  _el.addEventListener('click', function(e){
    // pass associated modal to method
    var _m = Helper.getClosest(e.currentTarget, '.fds-modal');
    modal__close( _m );
  }, false);
});
...
function modal__show(m){
  ...
  _body.className = _body.className + ' fds-modal-scroll-fix';
...
function modal__close(m){
  ...
  // Fix double scrollbar issue
  var _body = document.getElementsByTagName('body')[0];
  _body.className = _body.className.replace(' fds-modal-scroll-fix','');
