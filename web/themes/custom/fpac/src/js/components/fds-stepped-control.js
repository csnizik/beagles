...
var steppedControl__elements = document.querySelectorAll('.fds-stepped-control--sticky');
...
function steppedControl__setStyle() {
  ...
  Helper.forEach(steppedControl__elements, function(index, value) {
    ...
    if (_scBottomPosition > 12) {
      ...
      if(!_el.classList.contains('fds-stepped-control--unstuck')){
        _el.classList.add('fds-stepped-control--unstuck');
      }
    } else {

      if(_el.classList.contains('fds-stepped-control--unstuck')){
        _el.classList.remove('fds-stepped-control--unstuck');
      }
    }
  });
}
...
if(steppedControl__elements.length){
  ...
  var steppedControl__modals = document.querySelectorAll('.fds-modal');
